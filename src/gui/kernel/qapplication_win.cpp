...
#define PEEKMESSAGE_IS_BROKEN 1
        ...
        MSG keyMsg;
        bool done = false;
        while (PeekMessage(&keyMsg, 0, WM_KEYFIRST, WM_KEYLAST, PM_NOREMOVE)) {
          if (keyMsg.time < mouseMsg.time) {
            if ((keyMsg.lParam & 0xC0000000) == 0x40000000) {
              PeekMessage(&keyMsg, 0, keyMsg.message, keyMsg.message, PM_REMOVE);
            }
            else {
              done = true;
              break;
  ...
  if (type == QEvent::MouseMove || type == QEvent::NonClientAreaMouseMove || type == QEvent::TabletMove) {
    ...
    POINT curPos = msg.pt;
    if (curPos.x == gpos.x && curPos.y == gpos.y) return true;// same global position
    gpos = curPos;
  ...
  if (qApp->d_func()->inPopupMode()) {                        // in popup mode
...
#ifndef Q_OS_WINCE
    if (type == QEvent::MouseButtonPress && QApplication::activePopupWidget() != activePopupWidget && ptrTrackMouseEvent && curWin) {
    ...
    if ((type == QEvent::MouseButtonPress || type == QEvent::MouseButtonDblClick) && bs == button) {
      Q_ASSERT(testAttribute(Qt::WA_WState_Created));
      if (QWidget::mouseGrabber() == 0)
    ...
    const QPoint globalPos(gpos.x, gpos.y);
    QWidget* widget = QApplicationPrivate::pickMouseReceiver(this, globalPos, pos, type, Qt::MouseButtons(bs), qt_button_down, alienWidget);
    ...
    QMouseEvent e(type, pos, globalPos, Qt::MouseButton(button), Qt::MouseButtons(state & Qt::MouseButtonMask),
      Qt::KeyboardModifiers(state & Qt::KeyboardModifierMask));
    ...
    if (type == QEvent::MouseButtonRelease && button == Qt::RightButton) {
      QContextMenuEvent e2(QContextMenuEvent::Mouse, pos, globalPos, qt_win_getKeyboardModifiers());
