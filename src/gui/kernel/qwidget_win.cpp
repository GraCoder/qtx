...
QPoint QWidget::mapFromGlobal(const QPoint& pos) const
{
  ...
  if (!isVisible() || parentWindow->isMinimized() || !testAttribute(Qt::WA_WState_Created) || !internalWinId()
    ...
    if (extra && extra->topextra && extra->topextra->embedded) {
      ...
      // Adjust for window decorations
      QPoint fpt = parentWindow->frameGeometry().topLeft();
      fromGlobal -= parentWindow->geometry().topLeft() - fpt;
      return fromGlobal;
    }
  }
  POINT p;
...
void QWidgetPrivate::setWindowTitle_sys(const QString& caption)
{
  ...
  wchar_t* ptr = (wchar_t*)caption.utf16();
  SetWindowText(q->internalWinId(), ptr);
}
...
void QWidget::setWindowState(Qt::WindowStates newstate)
{
  ...
  if (isWindow()) {
    ...
    if ((oldstate & Qt::WindowMinimized) != (newstate & Qt::WindowMinimized)) {
      if (isVisible())
        ShowWindow(internalWinId(), (newstate & Qt::WindowMinimized) ? min : (newstate & Qt::WindowMaximized) ? max : normal);
    }
  }
    ...
    if (q->windowType() == Qt::Desktop) {
      ...
      if (w == 0 || h == 0) {
        ...
        if (isTranslucentWindow) {
          ...
          data.crect.setRect(rcNormalPosition.left + fStrut.left(), rcNormalPosition.top + fStrut.top(),
...
#endif
    top->frameStrut.setCoords(-rect.left, -rect.top - top->ncheight, rect.right, rect.bottom);
    data.fstrut_dirty = false;
  }
}
