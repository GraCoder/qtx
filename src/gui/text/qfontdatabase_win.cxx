...
{
  QLatin1String key("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows NT\\CurrentVersion\\FontSubstitutes");
...
static QFontEngine* loadEngine(int script, const QFontDef& request, HDC fontHdc, int dpi,
  const QtFontDesc* desc, const QStringList& family_list)
{
  ...
  if (!useDirectWrite) {
    ...
    if (SUCCEEDED(hr)) {
      QFontEngineDirectWrite* fedw = new QFontEngineDirectWrite(db->directWriteFactory, directWriteFontFace, request.pixelSize);
  ...
  if (script == QUnicodeTables::Common
    && !(request.styleStrategy & QFont::NoFontMerging) && desc != 0
...
QFontEngine* qt_load_font_engine_win(const QFontDef& request)
{
  ...
  if (fe != 0)
    ...
    return loadEngine(QUnicodeTables::Common, request, 0, qt_defaultDpi(), 0, QStringList());
}
...
static QFontEngine* loadWin(const QFontPrivate* d, int script, const QFontDef& req)
{
  ...
  while (!fe) {
    ...
    if (!desc.family)
      break;
    fe = loadEngine(script, req, d->hdc, d->dpi, &desc, family_list);
    if (!fe)
